...
class EmbeddingService:
    ...
    def embed_sparse(self, text: str) -> Dict:
        ...
        return {"indices": sparse_vector.indices.tolist(), "values": sparse_vector.values.tolist()}
    ...
    def _save_to_cache(
        self, cache_key: str, dense_vectors: List[List[float]], sparse_vectors: List[List[float]]
    ):
        """
        ...
        try:
            ...
            with open(file=cache_path, mode="wb") as f:
                pickle.dump(
                    {"dense": dense_vectors, "sparse": sparse_vectors},
                    f,
