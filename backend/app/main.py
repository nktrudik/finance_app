...
from app.core.qdrant_client import get_qdrant_client, create_collection
...
async def upload_csv(
    ...
    if old_count > 0 and not replace:
        logger.info(f"ℹУ пользователя уже есть {old_count} транзакций. Будут добавлены новые (дубликаты перезапишутся).")
